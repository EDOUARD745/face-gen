...
from .data import make_dataset, SKIN_GROUPS, denormalize_age
from .models.cgan import Generator
from .train_classifier import AttributeClassifier


def random_attrs(n, device, rng):
    return {
        "age": torch.tensor(rng.uniform(0, 1, n), dtype=torch.float32,
                            device=device),
...
def main():
    ...
    gen = load_generator(args.model, args.ckpt, args.image_size, device)
    ...
    for _ in range(10):  # 10 conditions fixes, 8 visages chacune
        ...
        x = gen(attrs8, 8, guidance_scale=args.guidance_scale)
        with torch.no_grad():
    ...
    n_eval, correct_g, correct_s, age_mae = 1000, 0, 0, 0.0
    done = 0
    while done < n_eval:
